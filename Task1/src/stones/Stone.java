...
public class Stone extends NobleMineral implements Comparable<Stone> {
    ...
    private final String DATA_FOR_CALC_INVALID_MSG = "Values has to be more than 0";
    private final String JEWELLING_NULL_MSG = "Stones jewelling is not defined";
    ...
    private void calculateCostOfStone(int costPerCarat) throws StonesCostCalculatingException {
        if ((double) costPerCarat == 0.0 || getCarats() == 0.0 || getRarity() == 0.0)
            throw new StonesCostCalculatingException(DATA_FOR_CALC_INVALID_MSG);
        this.cost = (double) costPerCarat * getCarats() / getRarity();

        if (isJewel == null) throw new StonesCostCalculatingException(JEWELLING_NULL_MSG);
        if (isJewel) {
